...
One founder, working with an AI assistant. You will see a second name on
recent commits and a second entry under Contributors, and it is put there on
purpose rather than left to be discovered.

The division is worth stating exactly, because the two halves are not the
same kind of work. Every decision that cannot be undone is the founder's:
the 22,000,000 cap and how it is split, the five-year lock on the founder
reserve, the 5% treasury that expires at block 400,000, the launch date, the
refusal to take investors or run a presale, and the rule that nothing is
published here in stronger words than the evidence supports. The code,
the tooling and most of the test scripts are written with Claude, at that
direction and under that review.

The commits carry a `Co-Authored-By` line only from 26 August onward. Before
that the same arrangement was in place and the line was simply not being
written, so the log understates it rather than the reverse. Correcting that
would mean rewriting 121 commits, changing every hash and breaking every
tag, and the record is not worth that -- so it is said here in plain words
instead.

Some people read "AI" as a weakness in a project. The weakness they have in
mind is real and worth naming precisely: code nobody checked, claims nobody
recomputed, volume without judgement. That is a weakness in any repository,
whoever typed it.

So the answer here is not an argument, it is the commands:

| The objection | What answers it |
|---|---|
| Nobody recomputed the economics | `python3 scripts/verify_supply.py` replays the 22,000,000 cap in exact integer arithmetic, block by block |
| Nobody knows what was changed from Bitcoin | `python3 scripts/patch_upstream.py --list` prints every difference with the reason for each — there are no `.patch` files, so nothing can be changed without appearing there |
| Nobody outside looked | two developers did, found two real security holes, and the whole exchange is in [review/REVIEW_RESPONSE.md](review/REVIEW_RESPONSE.md) |
| Nobody tests it | 24 automated checks run before anything is announced, and the summary states what was **not** run rather than letting a gap look like a pass |
| It has never been attacked properly | true, and said in bold three paragraphs above: **no paid security audit** |

Ask those five questions of any coin you are considering, and compare the
answers. That is the comparison this project would like to be judged on.
