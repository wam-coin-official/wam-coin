...
def main():
    ...
    g = sys.stdout
    indir = sys.argv[1]
    g.write('// Copyright (c) 2026 The WAM Coin developers\n')
    g.write('// Distributed under the MIT software license, see COPYING.\n')
    g.write('\n')
    g.write('#ifndef WAM_CHAINPARAMSSEEDS_H\n')
    g.write('#define WAM_CHAINPARAMSSEEDS_H\n')
    g.write('/**\n')
    g.write(' * Fixed seed nodes for WAM Coin.\n')
    g.write(' * AUTOGENERATED by contrib/seeds/generate-seeds.py -- do not edit.\n')
    g.write(' *\n')
    g.write(' * Each line is a BIP155 serialized (networkID, addr, port) tuple.\n')
    g.write(' *\n')
    g.write(' * Regenerate:\n')
    g.write(' *     python3 contrib/seeds/generate-seeds.py contrib/seeds \\\n')
    g.write(' *         > src/wam/chainparamsseeds.h\n')
    g.write(' *\n')
    g.write(' * Only main and test are emitted. WAM has no testnet4 and no signet\n')
    g.write(' * of its own -- CUnusedNetParams exists only so that upstream code\n')
    g.write(' * expecting those constructors still links -- and regtest is local.\n')
    g.write(' * All three keep vFixedSeeds.clear(). Emitting an array nothing uses\n')
    g.write(' * would be a zero-length array, which is not valid ISO C++.\n')
    g.write(' */\n')
    ...
    g.write('\n')
    g.write('#endif // WAM_CHAINPARAMSSEEDS_H\n')
